...
double p[MAX_K];
double kappa[2 * MAX_K];
/* Hyperexponentially distributed sizes:
 * A type-k job has mean size
 * 1 / kappa[2*k] with probability p[k]
 * and 1 / kappa[2*k+1] with probability 1 - p[k]
 */
...
int xik[MAX_I][2 * MAX_K];
/* Number of jobs of each type and of each size at each computer:
 * xik[i][2*k] = number of type-k jobs with mean size 1 / kappa[2*k] at computer i
...
/* Circular buffer encoding the queue */
...
static inline int draw_departure_computer () {
  int i = 0;
  double u = theta * uniform();
  double v = (xi[0] > 0) ? thetai[0] * mui[0] / xi[0] : 0.;

  while (u > v) {
    ++i;
    v += (xi[i] > 0) ? thetai[i] * mui[i] / xi[i] : 0.;
  }
...
static inline int draw_departure_type (int i) {
  int k = 0;
  double u = thetai[i] * uniform();
  double v = thetaik[i][0];
...
static inline void update_departure (int i) {
  int j, k;

  /* update the departure rate from computer i */
  thetai[i] = 0.;
  if (xi[i] > 0) {
    for (k = 0 ; k < K ; ++k) {
      thetai[i] += ( thetaik[i][2*k] = xik[i][2*k] * kappa[2*k] );
      thetai[i] += ( thetaik[i][2*k+1] = xik[i][2*k+1] * kappa[2*k+1] );
    }
  }

  /* update the total departure rate */
  theta = 0.;
  for (j = 0 ; j < I ; ++j)
    theta += (xi[j] > 0) ? thetai[j] * mui[j] / xi[j] : 0.;
}

static inline int jump () {
  int i, k;

  if ( (nu * rho + theta) * uniform() < nu * rho ) {
    ...
    if ( (i = assignment[k]) != -1 ) {
      ...
      remove_from_queue(i);
      ++xi[i]; ++xk[k];
      if (uniform() < p[k]) ++xik[i][2*k];
      else ++xik[i][2*k+1];

      /* update the departure rates */
      update_departure(i);
...
void simulation () {
  ...
  for (i = 0 ; i < I ; ++i) {
    ...
    thetai[i] = 0.;
    for (k = 0 ; k < K ; ++k) {
      xik[i][2*k] = 0; xik[i][2*k+1] = 0;
      thetaik[i][2*k] = 0.; thetaik[i][2*k+1] = 0.;
    }
    Li[i] = 0.;
    etai[i] = 0.;
  }
...
void read_inputs (int argc, char **argv) {
  ...
  /* read arguments */
  opterr = 0;
  while ( (c = getopt(argc, argv, "f:k:i:l:c:r:s:m:d:R:w:t:h")) != -1 ) {
    switch(c) {
      case 'f':
        /* output file name */
      ...
      case 's':
        ...
        for (k = 0 ; k < K ; ++k) {
          if (arg == NULL) {
            fprintf(stderr, "Option -s: you didn't specify all the sizes.\n");
            exit(EXIT_FAILURE);
          }
          mean = atof(arg);
          arg = strtok(NULL, " ");
          ...
          p[k] = 1. / (ratio + 1.);
          kappa[2*k] = 1 / (mean * ratio);
          kappa[2*k+1] = ratio / mean;
      ...
      case 'h':
        ...
        printf("\nOptions:\n"
            ...
            "    (default value: 10^6)\n"
            "-s: mean job size within each type\n"
            "\n");
        exit(0);
        break;
      case '?':
...
void print_inputs () {
  ...
  printf("\n--- DYNAMIC - SIMULATION - HYPEREXPONENTIAL ---\n\n");
